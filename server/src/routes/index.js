...
router.get('/api/regions', (req, res, next) => {
...
router.get('/api/region/:regionName', (req, res, next) => {
...
router.get('/api/storage', (req, res, next) => {
...
router.get('/api/journaling/:regionName', (req, res, next) => {
...
router.get('/api/replication', (req, res, next) => {

});

router.get('/api/processes/:commandName', (req, res, next) => {
    ...
    cp.exec("ps -C " + req.params.commandName + " -o pid:1,rss:1,pcpu:1,start_time:1,euid:1,cmd:1 --no-headers",
        {shell: "/bin/bash"},
        (err, stdout, stderr) => {
            if(stdout.length) {
                var ps = stdout.split("\n");
